...
def __(transform):
    transform["makery_url"] = transform.apply(
        lambda row: "https://makery.gogocarto.fr/map#/fiche/{name}/{id}/".format(
            name=str(row["name"]).replace(" ", "-"), id=row["makery_id"]
        ),
        axis=1,
    )
    return
...
@app.cell
def __(real_spaces):
    real_spaces["address_composed"] = (
        real_spaces["address.streetNumber"].astype(str)
        + ", "
        + real_spaces["address.streetAddress"].astype(str)
        + ", "
        + real_spaces["address.addressLocality"].astype(str)
        + ", "
        + real_spaces["address.postalCode"].astype(str)
        + ", "
        + real_spaces["address.addressCountry"].astype(str)
    )
    return
